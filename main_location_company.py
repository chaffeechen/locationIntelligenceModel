...
from gunlib.company_location_score_lib import global_filter, sub_rec_similar_company, sub_rec_condition, \
    merge_rec_reason_rowise, reason_json_format
...
def main():
    ...
    if args.mode == 'train':
        ...
        for ind_city in range(5):
            ...
            loc_feat = loc_feat.merge(
                comp_loc[['atlas_location_uuid']].groupby('atlas_location_uuid').first().reset_index(),
            ...
            sub_loc_recall = merge_rec_reason_rowise(sub_loc_recall, group_cols=['atlas_location_uuid'],
                                                     merge_col='reason')
            if wework_location_only:
                sub_loc_recall = sub_loc_recall.merge(sub_loc_feat_ww[['atlas_location_uuid']],
                                                      on='atlas_location_uuid', how='inner', suffixes=['', '_right'])
